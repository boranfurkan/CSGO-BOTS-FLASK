...
function checkNewItems(csgoEmpireMarketData, steamInventories, buffData, shadowpayMarketData, csgoMarketData, buffRate) {
    let source = new EventSource("/get-new-auctions");
    source.onmessage = function(event) {
        if (!event.data.startsWith("Error")) {
            let cleanData = event.data.replace(/(\r\n|\n|\r)/gm, "");
            let newItems = JSON.parse(cleanData)

            for (let item in newItems) {
                if (!steamInventories[item]) {
                    let values = newItems[item];
                    if (values["price"] > 40) {
                        if (buffData[item]) {
        ...
        showToast("Error", event.data, "warning")}
    };
}

$(document).ready(function(){
    // Get the input field and cards
let searchField = document.getElementById('item-search');
...
searchField.addEventListener('keyup', function() {
    ...
    cards.forEach(function(card) {
        ...
        // Check if the card name includes the search value
